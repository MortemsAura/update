...
import urllib.parse, urllib.request, re, json, requests, webbrowser, aiohttp, dns.name, asyncio, functools, logging, youtube_dl
...
async def on_connect():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}RAGNAROK {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
    ...
    '''+Fore.RESET)
    ctypes.windll.kernel32.SetConsoleTitleW(f'[RAGNAROK Selfbot v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
    headers = {
      'Authorization': token,
...
@Alucard.command()
async def youtube_dl(self, ctx, kwargs):
    await ctx.message.delete()
    ydl_opts = {
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="FUCK YOU CUNT")
        except:
            return
...
async def gcn(ctx):
    ...
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "ogoerugerogebgobgogoerbgoerbgerogboebgoerbgro"
        name = ""
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://pastebin.com/R2kULxiS'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
